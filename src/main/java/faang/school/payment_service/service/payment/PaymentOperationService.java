...
import faang.school.payment_service.exception.UnsupportedPaymentStatusException;
    ...
    public void updatePaymentOperation(PaymentOperationDto paymentData) {
        try {
            ...
            PaymentUpdateStrategy strategy = Optional.ofNullable(strategies.get(targetStatus))
                    .orElseThrow(() -> new UnsupportedPaymentStatusException("Unsupported payment status: " + targetStatus));
            PaymentOperation payment = paymentOperationRepository
                    .findByIdAndStatus(paymentData.getId(), requiredCurrentStatus)
                    .orElseThrow(() -> new PaymentOperationNotFoundException(
        ...
        } catch (UnsupportedPaymentStatusException | PaymentOperationNotFoundException e) {
